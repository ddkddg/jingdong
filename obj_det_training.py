...
parser.add_argument('--name', default = 'mytraintry')
...
if '3_fold' in args.split:
    ...
    train_split_seqs = ['MOT17-04-FRCNN']
    test_split_seqs = ['MOT17-02-FRCNN']
    # train_split_seqs = [
...
data_loader_no_random = torch.utils.data.DataLoader(
    dataset_no_random, batch_size=args.batch_size, shuffle=False, num_workers=0,
    collate_fn=utils.collate_fn)
...
model, bb, bbb = get_detection_model(dataset.num_classes, args.arch)
...
head = copy.deepcopy([bbb.body.layer3, bbb.body.layer4])
head += [nn.AvgPool2d(kernel_size=8, stride=8, padding=0), ViewFlatten(), nn.Linear(in_features=5 * 6144, out_features= ssh_classes, bias=True)]
...
params = [p for p in model.parameters() if p.requires_grad] + [p for p in bb.parameters()] + [p for p in ssh.parameters()]
...
for epoch in range(start_epoch, args.num_epochs + 1):
    ...
    loss_dicts = train_one_epoch(
        ssh, bb, model, optimizer, data_loader, device, epoch, print_freq=1)
    print("****************************")
    ...
    if args.eval_interval > 0 and (epoch % args.eval_interval == 0 or epoch in [1, args.num_epochs]):
        ...
        for metric, metric_value in evaluation_metrics.items():
            ...
            if metric_value > best_eval_metrics[metric]:
                ...
                torch.save({'model': model.state_dict()}, osp.join(output_dir, f"best_{metric}.model"))
                torch.save({'model': ssh.state_dict()}, osp.join(output_dir, f"best_{metric}_ssh.model"))
    ...
    if args.save_model_interval > 0 and epoch % args.save_model_interval == 0:
        ...
        torch.save({'model': ssh.state_dict()}, osp.join(output_dir, f"epoch_{epoch}_ssh.model"))
    torch.save({'model': model.state_dict()}, osp.join(output_dir, "last.model"))
    torch.save({'model': ssh.state_dict()}, osp.join(output_dir, "last_ssh.model"))
