...
class Tracker:
    ...
    def __init__(self, obj_detect, reid_network, tracker_cfg, ssh, bb):
    ...
    def test_single(self, model, ssh, image, label):
        model.eval()
        inputs, _ = self.obj_detect.transform(image, None)
        with torch.no_grad():
            outputs = model(inputs.tensors.to(device))
            outputs = ssh(outputs.to(device))
            #print(outputs.shape)
    ...
    def step(self, blob, optimizer_ssh):
        """This function should be called every timestep to perform tracking with a blob
        containing the image information.
        """
        #print(blob['img'].shape)
        # import torch.optim as optim
        # optimizer_ssh = optim.SGD([p for p in self.bb.parameters()], lr=0.001)
        #print(self.obj_detect.backbone.state_dict())
        #mine changes
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        criterion_ssh = nn.CrossEntropyLoss().to(device)
        images = blob['img'].to(device)
        #print(self.bb[0])
        #cor, con = self.test_single(self.bb, self.ssh, images, 0)
        ...
        def adapt_single(inputs):
            ...
            for iteration in range(2):
                ...
                inputs_ssh, labels_ssh = change_rgb_batch(inputs, 'rand')
                inputs_ssh, labels_ssh = inputs_ssh.to(device), labels_ssh.to(device)
                inputs_ssh, _ = self.obj_detect.transform(inputs_ssh, None)
                optimizer_ssh.zero_grad()
                #print(inputs_ssh.shape)
                outputs_ssh = self.bb(inputs_ssh.tensors)
                outputs_ssh = self.ssh(outputs_ssh)
                #print(outputs_ssh)
                #print(labels_ssh)
                ind, val = outputs_ssh.max(1)
                if val == labels_ssh:
                    print('right')
                else:
                    print('wrong')
                ...
                #print(loss_ssh)
                #loss_ssh.requires_grad_(True)
                loss_ssh.backward()
                optimizer_ssh.step()

        #inputs_ssh, _ = self.obj_detect.transform(images, None)
        #print(self.obj_detect.backbone)
        # f1 = self.obj_detect.backbone.body.conv1(inputs_ssh.tensors)
        # f1 = self.obj_detect.backbone.body.bn1(f1)
        # f1 = self.obj_detect.backbone.body.relu(f1)
        # f1 = self.obj_detect.backbone.body.maxpool(f1)
        # f1 = self.obj_detect.backbone.body.layer1(f1)
        # f1 = self.obj_detect.backbone.body.layer2(f1)
        #[backbone.body.conv1, backbone.body.bn1, backbone.body.relu, backbone.body.maxpool, backbone.body.layer1, backbone.body.layer2]
        #f1 = self.ssh(f1)
        adapt_single(images)

        # f2 = self.obj_detect.backbone.body.conv1(inputs_ssh.tensors)
        # f2 = self.obj_detect.backbone.body.bn1(f2)
        # f2 = self.obj_detect.backbone.body.relu(f2)
        # f2 = self.obj_detect.backbone.body.maxpool(f2)
        # f2 = self.obj_detect.backbone.body.layer1(f2)
        # f2 = self.obj_detect.backbone.body.layer2(f2)
        # if not f1.equal(f2):
        #     print('chgd here')
        ...
        # with open("./seeresult.txt","a") as f:
        #     l = scores.tolist()
        #     for _ in l:
        #         f.write(str(_))
        #         f.write(" ")
